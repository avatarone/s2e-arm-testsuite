Feature: Annotations - Check that intercepting memory reads works
    ...
    Scenario: Check that injector is not invoked for not selected memory ranges
        Then the stdout should not contain "2: address = 0x908, size = 0x4, is_io = false, is_code = false"
    ...
    Scenario: Check that the MemoryInterceptor plugin is invoked for code and data reads
        ...
        And the file "s2e-last/debug.txt" should contain "[MemoryInterceptor] slotMemoryRead called with address = 0x900 [concrete], access_type = 0xa1, size = 32, is_io = 0, is_code = 0" 
        And the file "s2e-last/debug.txt" should contain "[MemoryInterceptor] slotMemoryRead called with address = 0x908 [concrete], access_type = 0xa1, size = 32, is_io = 0, is_code = 0"
