Feature: Annotations - Check that intercepting memory reads works
    ...
    Scenario: Write annotation is added
        Then the file "s2e-last/debug.txt" should contain "[MemoryInterceptorAnnotation] Adding annotation for memory range 0x900-0x904 with access type 0x1f1aa, read handler '', write handler 'ann_write_intercept'"
    ...
    Scenario: Write annotation is invoked
        ...
        And the file "s2e-last/debug.txt" should contain "[MemoryInterceptor] slotMemoryWrite called with address = 0x900 [concrete], access_type = 0x40aa, is_io = 0"
    ...
    Scenario: Write annotation is not invoked for not selected memory ranges
        ...
        And the file "s2e-last/debug.txt" should contain "[MemoryInterceptor] slotMemoryWrite called with address = 0x908 [concrete], access_type = 0x40aa, is_io = 0"
