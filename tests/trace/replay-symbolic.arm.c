...
extern void s2e_print_expression(int expr, int dummy, const char * msg);
...
void _start() {
	...
	tmp = *a;
	symb_tmp = tmp * 0x20;
	s2e_print_expression(symb_tmp, 0, "first_var");
	if (tmp != 0xdeadbeef)
	...
	symb_tmp = tmp+0x20;
	symb_tmp++;
	s2e_print_expression(symb_tmp, 0, "second_var");
	if (tmp != 0xbabe)
		s2e_kill_state(2, "ERROR: second read returned wrong result");

	tmp = *c;
	symb_tmp = tmp+0x20;
	s2e_print_expression(symb_tmp, 0, "third_var_not_zero");
	symb_tmp = 0x0;
	s2e_print_expression(symb_tmp, 0, "third_var_zero");
	if (tmp != 0x13)
